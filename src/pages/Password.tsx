...
export default function Password() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (showForgotPassword) {
      ...
      // regular change flow (attempt update)
      if (!formData.currentPassword || !formData.newPassword || !formData.confirmPassword) {
        setError('Please fill in all password fields');
        return;
      }
              ...
              <form onSubmit={handleSubmit} className="p-6">
                {showForgotPassword ? (
                  <>
                    <div className="mb-4">
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          value={forgotPasswordEmail}
                          onChange={(e) => setForgotPasswordEmail(e.target.value)}
                          className="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#0B4F6C] outline-none text-sm"
                          placeholder="Enter your email address"
                          disabled={otpSent}
                        />
                      </div>
                    ...
                    {!otpSent ? (
                      <button type="button" onClick={handleSendOTP} disabled={loading} className="w-full bg-gradient-to-r from-[#0B4F6C] to-[#159A9C] text-white py-2.5 rounded-lg font-semibold hover:shadow-lg transition-all">Send OTP</button>
                    ) : !otpVerified ? (
                      <>
                        <div className="mb-4">
                          <label className="block text-sm font-semibold text-gray-700 mb-2">Enter OTP</label>
                          <div className="flex gap-2">
                            <input type="text" value={otp} onChange={(e) => setOtp(e.target.value)} className="flex-1 px-4 py-2.5 border border-gray-300 rounded-lg" placeholder="Enter 6-digit OTP" maxLength={6} />
                            <button type="button" onClick={handleVerifyOTP} disabled={loading || otp.length !== 6} className="px-4 py-2.5 bg-[#01796F] text-white rounded-lg">Verify</button>
                          </div>
                        </div>
                      </>
                    ) : (
                      <>
                        <div className="mb-4">
                          ...
                          <div className="relative">
                            <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" />
                            <input type={showNewPassword ? 'text' : 'password'} name="newPassword" value={formData.newPassword} onChange={handleChange} className="w-full pl-10 pr-12 py-2.5 border border-gray-300 rounded-lg" placeholder="Enter new password" />
                            <button type="button" onClick={() => setShowNewPassword(!showNewPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400">{showNewPassword ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}</button>
                          </div>
                        </div>
                        <div className="mb-6">
                          ...
                          <div className="relative">
                            <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" />
                            <input type={showConfirmPassword ? 'text' : 'password'} name="confirmPassword" value={formData.confirmPassword} onChange={handleChange} className="w-full pl-10 pr-12 py-2.5 border border-gray-300 rounded-lg" placeholder="Confirm new password" />
                  ...
                  <>
                    <div className="mb-4">
                      ...
                      <div className="relative">
                        <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" />
                        <input type={showCurrentPassword ? 'text' : 'password'} name="currentPassword" value={formData.currentPassword} onChange={handleChange} className="w-full pl-10 pr-12 py-2.5 border border-gray-300 rounded-lg" placeholder="Enter current password" />
                        <button type="button" onClick={() => setShowCurrentPassword(!showCurrentPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400">{showCurrentPassword ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}</button>
                      </div>
                    </div>
                ...
                <div className="flex gap-3">
                  <button type="submit" disabled={loading} className="flex-1 bg-gradient-to-r from-[#0B4F6C] to-[#159A9C] text-white py-2.5 rounded-lg font-semibold hover:shadow-lg transition-all">
                    {loading ? 'Saving...' : showForgotPassword && otpVerified ? 'Update Password' : showForgotPassword ? 'Continue' : 'Save Changes'}
                  </button>

                  {!showForgotPassword ? (
                    <button type="button" onClick={() => setShowForgotPassword(true)} className="flex-1 bg-white text-[#01796F] border border-[#01796F] py-2.5 rounded-lg font-semibold">Forgot Password?</button>
                  ) : (
                    <button type="button" onClick={() => { setShowForgotPassword(false); setOtpSent(false); setOtpVerified(false); setOtp(''); setForgotPasswordEmail(user?.email || ''); setError(''); setSuccess(''); }} className="flex-1 bg-white text-[#01796F] border border-[#01796F] py-2.5 rounded-lg font-semibold">Back</button>
                  )}
                </div>
              </form>
            </div>
