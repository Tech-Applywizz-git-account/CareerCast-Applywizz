...
const SignupPage: React.FC = () => {
  ...
  const [isPromoCodeLocked, setIsPromoCodeLocked] = useState(false);
  ...
  // Auto-fill promo code from URL ref param & Detect Location
  useEffect(() => {
    const searchParams = new URLSearchParams(location.search);
    const refCodeEncoded = searchParams.get("ref");

    let promoCodeToUse = null;

    // 1. Check URL param (Encoded)
    if (refCodeEncoded) {
      // Strict Base64 validation to prevent tampering (e.g. appended characters)
      const base64Regex = /^[A-Za-z0-9+/]*={0,2}$/;
      if (!base64Regex.test(refCodeEncoded)) {
        console.warn("Invalid characters in referral code");
      ...
      try {
        // Decode Base64
        const decodedString = atob(refCodeEncoded);

        // Verify format: Code|ReversedCode
        if (decodedString.includes('|')) {
          const [code, checksum] = decodedString.split('|');

          // Check if checksum matches (simple reverse check)
          if (code && checksum && checksum === code.split('').reverse().join('')) {
            promoCodeToUse = code;
            // Store in cookie for 30 days
            setCookie('referral_code', code, 30);
          } else {
            console.warn("Invalid referral code signature - tampering detected");
            setIsInvalidReferral(true);
            return;
          }
        } else {
        ...
        console.error("Failed to decode promo code", e);
        setIsInvalidReferral(true);
        return;
      }
    }
    // 2. If no URL param, check cookie
    else {
      const cookieCode = getCookie('referral_code');
      if (cookieCode) {
        promoCodeToUse = cookieCode;
      }
    }

    if (promoCodeToUse) {
      setForm((prev) => ({ ...prev, promoCode: promoCodeToUse }));
      // We don't need to lock it visibly since the field is hidden, but good for logic
      setIsPromoCodeLocked(true);

      // If no plan is set in state (direct link access), detect location
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 px-4">
        <div className="max-w-md w-full bg-white rounded-xl shadow-lg p-8 text-center">
          ...
          <h2 className="text-2xl font-bold text-slate-900 mb-2">Invalid Referral Link</h2>
          <p className="text-slate-600 mb-8">
            The referral link you used appears to be invalid or has been tampered with. Please check the link and try again.
          </p>
          <button
            onClick={() => navigate('/')}
